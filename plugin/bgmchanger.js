...
let baseURI = '/apps/' + config.HEROKU.APP_NAME;
 ...
 var BGM_TWO = ''
  ...
  if (config.LANG == 'EN') {
    ...
    mukki_desc = 'change reply message BGM mode'
    ...
    if (config.LANG == 'ML') {
      ...
      mukki_desc = 'മറുപടി bgm മോഡ് മാറ്റാൻ'
      BGM_ONE = '𝐁𝐆𝐌 തരം ഒന്നാം മോഡിലേക്ക് മാറ്റി'
      BGM_TWO = '𝐁𝐆𝐌 തരം രണ്ടാം മോഡിലേക്ക് മാറ്റി'
    }

 Asena.addCommand({pattern: 'bgm ?(.*)', fromMe: true, desc: mukki_desc, usage: '.bgm one / two' }, (async (message, match) => {
        if (match[1] == 'two') {
